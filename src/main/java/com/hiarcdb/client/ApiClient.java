...
import com.hiarcdb.client.auth.Authentication;
import com.hiarcdb.client.auth.HttpBasicAuth;
...
public class ApiClient {
    ...
    public ApiClient() {
        ...
        authentications.put("AdminApiKeyAuth", new ApiKeyAuth("header", "X-Hiarc-Api-Key"));
     ...
     * Set base path
     *
     * @param basePath Base path of the URL (e.g http://localhost:5000
    ...
    public Boolean checkResponse(Response response) throws ApiException {
    ...
    public <T> ApiResponse<T> executeDownload(Call call, Type returnType, String filepath) throws ApiException {
        try {
            InputStream body;
            Response response = call.execute();
            if (response.isRedirect()) {
                ...
                Response followedResponse = httpClient.newCall(request).execute();
                checkResponse(followedResponse);
                body = followedResponse.body().byteStream();
            } else {
                checkResponse(response);
                body = response.body().byteStream();
            }

            ReadableByteChannel rbc = Channels.newChannel(body);
            FileOutputStream fileOutputStream = new FileOutputStream(filepath);
            fileOutputStream.getChannel().transferFrom(rbc, 0, Long.MAX_VALUE);
            fileOutputStream.close();
            return new ApiResponse<T>(response.code(), response.headers().toMultimap());
        } catch (IOException e) {
            throw new ApiException(e);
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public <T> void executeDownloadAsync(Call call, final Type returnType, String filepath, final ApiCallback<T> callback) {
        call.enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                callback.onFailure(new ApiException(e), 0, null);
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                callback.onSuccess(null, response.code(), response.headers().toMultimap());
            }
        });
    }
     ...
     * @param returnType Return type
     * @return Type
     * @throws ApiException If the response has an unsuccessful status code or fail
    ...
    public Call buildDownloadCall(String path, String method, String filepath, List<Pair> queryParams,
